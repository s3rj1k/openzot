...
engine (agent harness, the loop with its model client, thread assembly, session logs) and
the `tui` viewer. Nothing is importable - zot is a binary, not a library - and
