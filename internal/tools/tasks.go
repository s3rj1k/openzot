...
// tasksTool records the task list. It changes nothing on disk. Its value is that the model must state what the work needs
// and where each part stands. Every call carries the whole list and replaces the last, so there is no state here, and the
// call is read through ParseTasks, so what the tool accepts and what the viewer shows cannot drift.
